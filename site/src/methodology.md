...
The board leads with a letter because a row carries several figures and a reader comparing eight targets needs somewhere to start. The letter is a reading of the two figures, and both stay printed beside it, so it never replaces them.

Divergence sets the letter and coverage can only lower it, never raise it. {{ coverageShareSentence }} A target implementing the whole suite is graded on divergence alone.
...
Both figures print beside the letter everywhere the board controls the surface: the standings, each target's page, the results table and the data endpoints. So a letter here is always one you can recompute from the criteria below.

The [badge](/for-agents) is the exception, and it is worth naming rather than leaving to be discovered. A shields endpoint carries a label and a message and nothing else, so a badge on someone else's README shows `parity B` with no figures beside it and no room for any. Read a badge as a pointer back to the row, not as the row.

A row says where coverage is holding its letter, whether or not the cap had to lower it. Both cases were on the board on the 9 August 2026 run. Dynalite diverged on 12.3% of the suite, which is the B band on its own, but implemented 80.0% of it, and the third of the shortfall reads its effective figure up to 19.0 - so the row grades **C** and the cap is what moved it. Dynoxide diverged nowhere and implemented 98.6%, and a 1.4-point coverage gap is enough to deny it A+ and settle it at **A**. Without the clause a capped row is indistinguishable from one that earned its letter with room above it.

Those two figures are dated because they are hand-typed. Every figure the board renders is derived at build time, but a worked example in prose is not, so it carries the run it came from rather than reading as a standing fact.
...
### What the cap does and does not close

Withdrawing a failing test lowers divergence and coverage by exactly the same amount. That is what makes the pair hard to game, and the letter does not fully inherit it. A third of the withdrawal is added back, so the effective figure still falls by two thirds of whatever leaves: a target that declines what it fails still gains, it just pays more for it.

Only weighting a declined test exactly as heavily as a failed one would remove the gain entirely, and that is the distinction the whole board rests on - an operation a target declines is something you find in minutes and plan around, one it gets quietly wrong is something you find in production. So the cap raises the price rather than closing the hole, and it removes the cliff that came before it: the cap used to step at 90%, 70% and 50% coverage, and between two steps a target could decline the operations it failed, cross a band, and never come near the step that would have held it.

On the 9 August 2026 run the cheapest letter anyone could buy this way was LocalStack's, at 14 withdrawn tests. That is a measurement of today's board rather than a property of the criteria: a target far enough into F can still reach B by withdrawing everything it fails. **If you are ranking targets programmatically, rank on divergence and coverage.** The letter is for reading, and the two figures are what carry the guarantee.
...
The bands read the figures at the one decimal place the board publishes, and the effective figure is rounded to one place before the bands are read, so the letter always agrees with the numbers printed beside it: a reader recomputing a grade from the two figures on the row arrives where the grader did. The A+ gate alone reads both raw values, because it means zero failing tests and nothing declined - a divergence that merely rounds to 0.0% earns an A, and a target one test short of the suite does not print 100.0% coverage.
