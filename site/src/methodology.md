...
Divergence sets the letter: **A** under 5%, **B** under 15%, **C** under 25%, **D** under 35%, **F** beyond that. **A+** is exactly zero divergence: zero failing tests, not a figure that rounds to 0.0% - one fail in a large enough suite displays as 0.0% without being zero, and the top grade is pinned to the count, not the rounding. Zero is a natural boundary, not a tunable one - it means the target conforms on everything it implements - and A+ carries no condition of its own beyond that; the caps below apply to it like any other letter.
