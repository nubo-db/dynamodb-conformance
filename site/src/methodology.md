...
Coverage caps the letter: a target implementing under 90% of the suite grades no better than **B**, under 70% no better than **C**, under 50% no better than **D**. The caps stop there: coverage alone never grades a target **F**, because F means wrong on more than a third of the suite, and an operation a target never attempts is absent, not wrong. A cap never changes a number. It ceilings the letter, the row says "capped by coverage" when it bites, and the coverage figure that earned the cap is printed right beside it. The live example is Dynoxide's WebAssembly build: zero divergence over 78.7% of the suite reads **B**, not A+.

The divergence boundaries at 5% and 25% are the colour bands this board has published since it began; the letters restate them, with interior splits at 15% and 35%, rather than drawing new lines.
...
Every letter on the board is produced by one shared function from the two published figures and nothing else - no target is graded by different rules, the baseline and the [board author's own engines](/targets/dynoxide) included - and the suite's tests assert exactly that: the published grade must equal the one recomputed from the row's own divergence and coverage. A row without "capped by coverage" is a letter that equals its divergence band.
